...
module.exports = {
  ...
  onChat: async function ({ args, message, event, getLang }) {
    if (args[0].trim().toLowerCase().startsWith('hadi') && args.length > 1) {
  ...
  detectLanguage: function (words) {
    ...
    const firstWord = words.find(word => word.startsWith('hadi')) || '';
    return firstWord.startsWith('hadi') ? 'en' : 'bn';
  },
